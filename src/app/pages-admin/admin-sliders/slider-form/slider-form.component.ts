    ...
    this.observ$ = this.adminSliderService.update(slider).pipe(
      ...
      .subscribe(
        json => {
          ...
          swal.fire('slider actualizado', `${json.mensaje}, label: ${json.slider.label}`, 'success');
        }
        , err => {
          if (err.status === 400) {
    ...
    this.observ$ = this.adminSliderService.create(slider).pipe(
      ...
      .subscribe(
        json => {
          ...
          swal.fire('creado slider, no olvide asociar una foto', `${json.mensaje}, label: ${json.slider.label}`, 'success');
        }
        , err => {
          if (err.status === 400) {
            this.erroresValidacion = err.error.errors as string[];
